...
void chol_solve_updated_kkt(double *rhs, cholmod_factor *L,
                            cholmod_common *cm, double *x
  , CSC *kkt_updated, CSC *H, CSC *B, CSC *C, std::vector<int> as,
   int outer_iter){
 size_t  n = L->n;
 int xtype = L->xtype;
 long *LPerm = (long*) L->Perm;
...
/* cholmod_dense *YY, *ZZ = cholmod_l_ones(n,1,CHOLMOD_REAL,cm) ;
 ...
 if(outer_iter > 0){
  ...
  kkt_updated->stype = -1; kkt_updated->packed = 1; kkt_updated->xtype = CHOLMOD_REAL;
  ...
  kkt_u_t_tmp->stype = 1; kkt_u_t_tmp->packed = 1; kkt_u_t_tmp->xtype = CHOLMOD_REAL;
  kkt_u_t_tmp->sorted = 1;

  kkt_u_t->stype = -1; kkt_u_t->packed = 1; kkt_u_t->xtype = CHOLMOD_REAL;
  kkt_u_t->sorted = 1;
