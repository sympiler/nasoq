...
#include <queue>
#include "symbolic_phase.h"
#include "parallel_blocked_ldlt.h"
#include "parallel_blocked_ldlt_02.h"
#include "parallel_blocked_ldlt_03.h"
#include "Serial_blocked_ldl.h"
#include "Serial_update_ldl.h"
#include "Serial_blocked_ldl_02_2.h"
#include "Serial_update_ldl_static.h"
#include "Parallel_update_ldl_02_2.h"
#include "serial_simplicial_ldl.h"
#include "Serial_update_simplicial_ldl.h"
#include "Parallel_simplicial_ldl.h"
#include "Parallel_update_simplicial.h"
#include "solve_phase.h"
#include "mgmres.hpp"
#include "Util.h"
#include "Norm.h"
#include "ldlt_check.h"
...
struct profiling_solver_info{
 ...
 profiling_solver_info(int nt):fact_time(0),analysis_time(0),
...
struct SolverSettings{
 ...
 SolverSettings(CSC *Amat, double *rhs_in, CSC* Bmat):
  ...
  BT->stype=B->stype;BT->xtype=B->xtype;BT->packed=B->packed;
  BT->nz = B->nz; BT->sorted = B->sorted;
  remove_trans=1;
  solver_mode=1;
 }
 ...
 void default_setting(){
  ...
  chunk = 1;
  cost_param=num_thread;level_param=-3;
  final_seq_node=50;// should be greater than 1
  n_relax[0]=4; n_relax[1]=16; n_relax[2]=48;
  z_relax[0]=0.8; z_relax[1]=0.1; z_relax[2]=0.05;
  //z_relax[0]=0.9; z_relax[1]=0.5; z_relax[2]=0.05;
  //refinemen
  req_ref_iter = 2;
  max_iter=req_ref_iter;
  max_inner_iter=2;
  tol_abs = 1e-15; tol_rel = 1e-15;
  //
  reg_diag = 1e-8;
  is_super=1;
  regularization = 1;
 ...
 */
 int build_super_matrix(){
  ...
  SM->p = SMp; SM->i = SMi;
  SM->x = SMx;
  SM->stype=-1;SM->xtype=CHOLMOD_REAL;SM->packed=TRUE;
  SM->sorted = TRUE; SM->nzmax = SM_nz;
 ...
 */
 ...
 void reorder_matrix(){ //TODO: developing a lightweight version of this.
...
/*  for (int i = A->ncol; i < A_ord->ncol; ++i) {
 ...
 int check_ldlt_factor(){ // TODO move it to unit test later
