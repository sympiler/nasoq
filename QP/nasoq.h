...
#include "qp_utils.h"
#include "osqp_utils.h"
#include "updown_test.h"
#include "Util.h"
#include "nasoq_utils.h"
...
Nasoq(size_t H_size, int *Hp, int *Hi, double *Hx, double *q_in,
 ...
 H = new CSC;
 H->nzmax = Hp[H_size]; H->ncol=H->nrow=H_size;
 H->stype=-1;H->xtype=CHOLMOD_REAL;H->packed=TRUE;
 H->p = Hp; H->i = Hi; H->x=Hx; H->nz = NULL;
 H->sorted = TRUE;
 hessian_size = H->nrow;
 //HT = new CSC;
 q = q_in;
 ...
 A = new CSC;
 A->nzmax = 0; A->ncol= 0; A->nrow=0;
 A->stype=1;A->xtype=CHOLMOD_REAL;A->packed=TRUE;
 A->p = NULL; A->i = NULL; A->x=NULL; A->nz = NULL;
 A->sorted = TRUE;
 AT = new CSC;
 a = NULL;
 ...
 B = new CSC;
  B->ncol=B_col ;B->nrow=B_row;
 B->stype = 0;B->xtype=CHOLMOD_REAL;B->packed=TRUE;
 B->p = Bp; B->i = Bi; B->x=Bx; B->nz = NULL;
 B->sorted = TRUE;
 BT = new CSC;
 b = b_ineq;
...
Nasoq(size_t H_size, int *Hp, int *Hi, double *Hx, double *q_in,
 ...
 A->stype=1;A->xtype=CHOLMOD_REAL;A->packed=TRUE;
 A->p = Ap; A->i = Ai; A->x=Ax; A->nz = NULL;
 A->sorted = TRUE;
 a = a_eq;
 if(A->nrow>0){
  A->nzmax = Ap[H_size];
...
void default_setting(){
 max_iter = 4000;
 reg_diag = 1e-9; zero_thresh = 1e-9;
